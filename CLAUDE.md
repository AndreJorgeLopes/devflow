...
at `.devflow/state/<branch>/<phase>.md`, gates on a one-click `AskUserQuestion`, then
emits a copy-pasteable resume prompt for the user to paste after `/clear`. The next
phase reads only the frozen-state file and the absolute artefact paths it lists as
source of truth — the brainstorming context does not bleed into implementation.
